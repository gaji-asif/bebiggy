...
$(document).on('keypress', '.chat-textarea', function (e) {
    ...
    if (message !== "" && e.which == 13) {
        ...
        // save the message 
        $.ajax({
            type: "POST", url: baseUrl + "chat/save_message", data: { message: message, user: $('#chat_buddy_id').val(), [csrfName]: csrfHash }, dataType: "json", cache: false,
...
$(document).on('submit', '.msgOwnerForm', function (e) {
    ...
    if (message !== "") {
        ...
        // save the message 
        $.ajax({
            type: "POST", url: baseUrl + "chat/save_message", data: { message: message, user: $('.owner_id').val(), [csrfName]: csrfHash }, dataType: "json", cache: false,
            success: function (response) {
...
function load_thread(user, limit = 5) {
    ...
    $.ajax({
        type: "POST", url: baseUrl + "chat/message", data: { user: user, limit: limit, [csrfName]: csrfHash }, dataType: "json", cache: false,
        success: function (response) {
