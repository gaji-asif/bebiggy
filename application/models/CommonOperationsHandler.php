...
class CommonOperationsHandler extends CI_Model
{
    ...
    {
        // if (!empty($this->session->userdata('user_id')) && ($this->session->userdata('user_level') == 1 || $this->session->userdata('user_level') == 0)) {
    ...
    public function create_invoice($data)
    {
        ...
        if (!empty($processing_rate) && $processing_rate > 0) {
            $without_fee        = ($data['amount'] / (100 + $processing_rate)) * 100;
            $processing_fee     = ($without_fee * $processing_rate) / 100;
        }
    ...
    public function _urlSlugGenerator($table, $id, $column, $slug)
    {
        ...
        if (isset($urlSlug) && !empty($urlSlug)) {
            ...
            if (isset($query->result_array()[0][$id])) {
                return $urlSlug . '-' . strtotime(date('Y-m-d H:i:s'));
                // if (!preg_match('/-{1}[0-9]+$/', $urlSlug)) {
                //     return $urlSlug .= '-' . ++$i;
                // } else {
                //     return $urlSlug = preg_replace('/[0-9]+$/', ++$i, $urlSlug);
                // }
            } else {
                return $urlSlug;
            }
    ...
    /*Expert Unique URL slug Generator*/
    public function _expertUrlSlugGenerator($slug)
    {
        $urlSlug = $this->urlSlug($slug);
        return $urlSlug . '-' . strtotime(date('Y-m-d H:i:s'));
    }
    ...
    /*Unique URL slug Check*/
    public function _urlSlugCheck($table, $id, $column, $slug, $tableId = 0)
    {
    ...
    public function generateUserMembershipLevel($user_id)
    {
        // first delete cache 
        $this->load->driver('cache');
        $userCache  = "user_" . $user_id;
        $this->cache->file->delete($userCache . "_course");
        $this->cache->file->delete($userCache . "_permission");
        // regenerate cache
        ...
        if (!empty($membership_level_id)) {
            ...
            fileCache($userCache . "_permission", $membership_permissions,  "save");
        }
    }


    public function refreshUserMembershipLevel($membership_level_id, $user_id)
    {
        // first delete cache 
        $this->load->driver('cache');
        $userCache  = "user_" . $user_id;
        ...
        $membership_permissions = 0;
        $membership_permissions = $this->database->getUserAllowePermission($membership_level_id);
        if (!empty($membership_permissions)) {
            fileCache($userCache . "_permission", $membership_permissions,  "save");
        }
    }
}
