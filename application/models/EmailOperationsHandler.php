...
class EmailOperationsHandler extends CI_Model
{
  ...
  function __construct() {
      ...
      switch ($type) {
          ...
          if(!empty($tempdata)){
            ...
            if(!empty($listing)){
              ...
              $data['detail']         = 'Amount '.$data['currency'].'  0 has been charged for the following invoice '.$tempdata['invoice_id'];
            }
            $data['template']       = $this->load->view('templates/invoice_email.tpl', $data, TRUE);
            $template               = $data['template']; 
    ...
    public function _send_contact_email(){
      ...
      // pre($data['settings'][0]['office_email']);
      // pre($data['settings'][0]['admin_email']);
      // pre($this->input->post('contact_email'));
      // pre($this->lang->line('site_name').' Contact Form -'.$this->input->post('contact_subject'));
      // pre($this->input->post('contact_msg'));
      if($this->email->send()){
        return true;
      }
      else
