...
class Main extends CI_Controller
{
	...
	public function home()
	{
		...
		// // Fetch Website
		// $data['featuredWebsite']	=	$this->database->_get_selected_listing_types('date', 0, RESULTS_PER_HOMEPAGE, array('tbl_listings.listing_type' => 'website'));
		...
		$data['home_type_1']			=   $this->database->_get_row_data('tbl_listings', array('listing_type' => 'home_type_1', 'status' => 1));
		pre($data, 1);
	...
	function courseDetail($id = 0)
	{
		if ($id != 0) {
			...
			$data['records'] 		= 	$this->database->_get_courses($id, '', '');
			$data['lessons'] 		= 	$this->database->_get_lession($table, $id,  '', '', $active);
	...
	{
		$this->loadPage('what-is-drop-shipping');
	}

	public function makeMoneyDropShipping() {
		$this->loadPage('make-money-drop-shipping');
	}

	public function startOnlineShop() {
		$this->loadPage('start-online-shop');
	}

	public function websitesForSale(){
		$this->loadPage('websites-for-sale');
	}

	public function getStarted(){
		$this->loadPage('get-started');
	}

	public function dropShipping()
	{
		$this->loadPage('drop-shipping');
	}
	...
	public function static_product($id) {
		...
		$data['single_item'] = $this->database->_get_row_data('tbl_frontend_products', array('id' => $id));
