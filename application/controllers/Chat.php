...
class Chat extends CI_Controller {
    ...
    public function index(){
	   ...
	   //pre($contacts);
		...
		foreach ($contacts as $key=>$contact) {
			...
			$unread 							= 	$this->chat->unread_per_user($this->session->userdata('user_id'),$contact); 
			//pre($unread ,1);
			$userListArr[$key]['from']			= 	$this->session->userdata('user_id');
			$userListArr[$key]['to']			= 	$this->user->get_user($contact);
			$userListArr[$key]['last_msg']		= 	$this->chat->get_last_msg($this->session->userdata('user_id'),$contact);
	...
	public function message()
	{
		...
		//pre($user);
		// $this->chat->mark_read_menual($user)

		$thread = array();
		foreach ($messages as $message) {
			$owner 	= $this->user->get_by('user_id',$message->sender_id);
			$this->chat->mark_read_menual($message->id);
			$chat = array(
				'msg' 		=> $message->id,
	    ...
	    if ($new_exists) {
			...
			foreach ($new_messages as $message) {
				...
				$chat = array(
					...
					'sender' 	=> $message->sender_id, 
					'recipient' => $message->recipient_id,
					'avatar' 	=> !empty($owner->thumbnail) ? $owner->thumbnail : 'no-image.jpg',
					'body' 		=> parse_smileys($message->message),
					'time' 		=> date("M j, Y, g:i a", strtotime($message->timestamp)),
					'type'		=> $message->sender_id == $user ? 'out' : 'in',
					'name'		=> $message->sender_id == $user ? 'You' : (isset($owner->firstname) ? ucwords($owner->firstname) : 'Anonymous') ,
					);
				array_push($thread, $chat);
			}
