				...
				<div class="dashboard-content-inner">
					...
					<div class="dashboard-headline">
						<div class="dashboad_table">
							...
							<h3><b>Manage Classified Listings </b></h3>
					...
					<div class="row">
						...
						<!-- Dashboard Box -->
						<div class="col-xl-12">
							<?php $this->load->view('user/includes/alert'); ?>
												...
												if (in_array($listing['listingType'], array_column($platforms, 'platform'))) {
													...
													if ($listing['status'] !== '1' || $listing['sold_status'] === '1') { ?>
														...
														<div class="domains-listing width-adjustment">
															...
															<div class="domains-listing-details">
																...
																<div class="domains-listing-description">
																	<h3 class="domains-listing-title"><a href="<?php //echo base_url().'single_auction/'.$listing['listingType'].'/'.$listing['id']; 
																												?>">
																			<?php if (isset($listing['website_BusinessName']))
																				echo $listing['website_BusinessName'];
																			else if (isset($listing['tagline'])) echo $listing['tagline']; ?></a> <span class="dashboard-status-button green">
																			<?php if (isset($listing['listingType'])) echo strtoupper($listing['listingType']); ?></span>
																		<?php if ($listing['sold_status'] !== '1') {
																			if ($listing['status'] === '1') { ?> <span class="dashboard-status-button green">ACTIVE</span></h3>
																<?php } else if ($listing['status'] === '3') { ?> <span class="dashboard-status-button yellow">EXPIRED</span>
																<?php } else if ($listing['status'] === '2') { ?> <span class="dashboard-status-button red">SUSPENDED</span>
																<?php } else if ($listing['status'] === '5') { ?><span class="dashboard-status-button red">UNVERIFIED</span>
																<?php }
																			if ($listing['status'] === '9') { ?><span class="dashboard-status-button yellow"><b>PENDING FOR APPROVAL</b></span>
																<?php }
																		} else {  ?> SOLD <?php } ?>
														...
														<ul class="dashboard-task-info">
															...
															<li><strong><?php if (!empty($default_currency)) echo $default_currency;
																		else echo '$ '; ?> <?php if (isset($listing['averageBid'])) echo ($listing['averageBid']);
																							else echo '0'; ?></strong><span>Avg. Offers</span></li>
															...
															<li><strong><?php if (!empty($default_currency)) echo $default_currency;
																		else echo '$ '; ?> <?php echo $listing['highestbid']; ?></strong><span>Highest Offer</span></li>
