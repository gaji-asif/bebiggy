...
<head>
	...
	<!--User Page Meta Tags-->
	<title>Transaction - <?php if (!empty($contract[0]['contract_id'])) {
								echo '#' . $contract[0]['contract_id'];
							} ?> | <?php echo $this->lang->line('site_name') ?>| User Dashboard</title>
	<meta charset="utf-8">
			...
			<div class="dashboard-content-container" data-simplebar>
				<div class="dashboard-content-inner">
					<div class="dashboad_table">
						...
						<h3>Open Transaction With You & <a href="<?php //echo current_url(); 
																	?>"><?php //if(isset($contract[0]['firstname'])) //echo $contract[0]['firstname'] 
																		?> <?php //if(isset($contract[0]['lastname'])) echo $contract[0]['lastname']; 
																			?><?php if (isset($contract[0]['contract_id'])) echo $contract[0]['contract_id']; ?></a></h3>

						<span class="margin-top-7"> <a href="<?php //echo current_url(); 
																?>"><?php //if(isset($contract[0]['contract_id'])) echo $contract[0]['contract_id']; 
																	?></a> <?php if ($contract[0]['status'] === '4' || $contract[0]['status'] === '7') { ?></span>
					...
					<?php if (isset($contract[0]['id']) && !empty($contract[0]['id'])) { ?>
						...
						<div class="dashboard-headline">
							...
							<span class="margin-top-7 payment_status_">
								...
								<div class="dashboard-box margin-top-0">
									<div class="content">
										<ul class="dashboard-box-list ">
											...
											<li class="domains-box active bg-white overflow-hidden border rounded mt-4 position-relative overflow-hidden contract_section_a">

												<div class="lable text-center pt-2 pb-2 box_contract_a">
														...
														<div class="seller-name">
															...
															<ul class="dashboard-task-info bid-info">
																<?php if ($contract[0]['bid_id'] === 'direct') { ?> <li><strong><?php if (!empty($default_currency)) echo $default_currency;
																																else echo '$'; ?> <?php if (!empty($contractamount)) echo number_format($contractamount); ?></strong><span>Transaction Amount</span></li><?php } else { ?> <li><strong><?php if (!empty($default_currency)) echo $default_currency;
																																																																										else echo '$'; ?><?php if (isset($biddata[0]['bid_amount'])) echo number_format($biddata[0]['bid_amount']); ?></strong><span><?php echo strtoupper($contract[0]['type']) ?> AMOUNT</span></li> <?php } ?>
										...
										<div id="negotiations_table" class="bs-example container" data-example-id="striped-table">
											<table class="table table-striped table-bordered table-hover">
												...
												<tbody>
													...
													<?php $i = 1;
													foreach ($contractsHistory as $contracts) { ?>
														<tr>
															<th scope="row"><?php echo $i; ?></th>
															<td><?php if ($contracts['status'] === '1') echo 'Paid';
																else if ($contracts['status'] === '2')
																	echo 'Support Review';
																else if ($contracts['status'] === '3')
																	echo 'Canceled Transaction';
																else if ($contracts['status'] === '6')
																	echo 'Requested a Revision';
																else if ($contracts['status'] === '4')
																	echo 'Completed';
																else if ($contracts['status'] === '5')
																	echo 'Delivered & Waiting for Approval';
																else if ($contracts['status'] === '7')
																	echo 'Canceled the Transaction and refunded';
																else if ($contracts['status'] === '8')
																	echo 'Rejected Cancel Request By Seller';
																else if ($contracts['status'] === '9')
									...
									<div class="dashboard-box margin-top-0">
										...
										?>
												...
												<div class="container-timeleft time_interval_a" id="container">
													...
													<h1 id="time" class="time_number_a">00:00:<span>00</span></h1>
													<h2 id="code"></h2>
													<?php if ($contract[0]['customer_id'] === $this->session->userdata('user_id')) { ?>
														<div class="button-group" id="action">
															<a href="#small-dialog-6" data-contractid="<?php if (isset($contract[0]['id'])) echo $contract[0]['id']; ?>" class="popup-with-send-message button dark ripple-effect"><i class="mdi mid-hand-okay"></i> Cancel Transaction</a>
														</div>
													<?php } ?>
												</div>
